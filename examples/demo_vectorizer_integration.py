"""
Демонстрационный скрипт для проверки Модуля 4 (Векторизация и векторная БД).

Выполняет реальный вызов OpenAI Embeddings API (тест 5) — требует
OPENAI_API_KEY в .env. Не запускается автоматически pytest'ом.
"""
